...
#' 2012 is the earliest data we are using because that is when full data sets
  ...
  tib_valid <- left_join(tib_valid, edu_data, by = c("GEOID", "state", "year")) %>%
    mutate(
      education_attainment = ifelse(twenty_five_and_older > 0,
                                    ...
                                    NA_real_),
      education_attainment = percent(education_attainment / 100, accuracy = 0.01)
    ) %>%
...
add_dissimilarity_index <- function(tib, minority_group_code = NULL)
{
  ...
  sums <- left_join(reference_num, subject_num, by = c("GEOID", "county", "state_abbr", "year")) %>%
    rename(tract_geos = GEOID) %>%
    rename(state = state_abbr)

  tib_valid <- tib_valid %>%
    left_join(sums, by = c("county", "state", "year"))
  ...
  tib_valid <- tib_valid %>%
   ...
   group_by(address, year) %>%
    ...
    ungroup() %>%
      select(-reference_sum, -subject_sum, -reference_group, -subject_group, -tract_geos, -difference) %>%
...
add_decennial_dissimilarity <- function(tib, minority_group_code_dhc = NULL, minority_group_code_sf1 = NULL)
{
  ...
  if("decennial_dissimilarity_index_dhc" %in% names(tib_valid) & "decennial_dissimilarity_index_sf1" %in% names(tib_valid))
  {
    tib_valid <- tib_valid %>%
      ...
      select(-decennial_dissimilarity_index_dhc, -decennial_dissimilarity_index_sf1)
  }
  else if("decennial_dissimilarity_index_dhc" %in% names(tib_valid))
  {
    tib_valid <- tib_valid %>%
      ...
      select(-decennial_dissimilarity_index_dhc)
  }
  else
  {
    tib_valid <- tib_valid %>%
      mutate(decennial_dissimilarity_index = decennial_dissimilarity_index_sf1) %>%
      select(-decennial_dissimilarity_index_sf1)
  }
  tib <- tib %>%
...
add_concentrated_poverty <- function(tib)
{
  ...
  tib_valid <- tib_valid %>%
    ...
    mutate(concentrated_poverty = ifelse(is.na(concentrated_poverty), 0, concentrated_poverty))
  tib <- tib %>%
    left_join(tib_valid %>%
...
#'
#' @description
#' ⚠️ **Note:* Users must install the RAQSAPI package, which will prompt for
#' valid credentials, necessary for the functionality of this function
#'
#' The `add_AQI()` function appends annual Air Quality Index (AQI) data from the
#' `RAQSAPI` package based on **geographic location**(derived from a provided
#' address) and year. It geocodes each address using **OpenStreetMap** via the
#' `tidygeocoder` package, identifies the nearest air quality monitoring site
#' reporting to the EPA's Air Quality System (AQS), retrieves pollutant-specific
#' data for the specified year, computes AQI values using official EPA
#' breakpoint formulas, and classifies each observation into a corresponding
#' health risk category. [Data from](https://cran.r-project.org/web/packages/RAQSAPI/index.html).
#'
#' @details
...
#' @importFrom tibble tibble
#' @importFrom magrittr %>%
#' @importFrom RAQSAPI aqs_monitors_by_box aqs_annualsummary_by_site
#'
#' @export
add_AQI <- function(tib)
  {
    ...
    coords <- tib %>%
      geocode(address, method = "osm", lat = lat, long = lon, full_results = TRUE)
    coords <- coords %>%
      filter(!is.na(coords$lat) | !is.na(coords$lon))
     if (nrow(coords) == 0)
    {
      stop("Failed to geocode the address.")
       return(NULL)
    }
    ...
    lon <- coords$lon
    year <- coords$year
    # Step 2: Define a small box (approx 0.5° buffer)
    buffer <- 0.1
    box <- tibble(
      min_lat = lat - buffer,
      max_lat = lat + buffer,
    ...
    for(i in 1:nrow(box))
    {
      individuals <- aqs_monitors_by_box(
        parameter = c(pm = "88101", co = "42101", no2 = "42602", o3 = "44201", so2 = "42401"),
        bdate = paste0(box$year[i], "0101"),
        edate = paste0(box$year[i], "1231"),
        minlat = box$min_lat[i],
        maxlat = box$max_lat[i],
        minlon = box$min_lon[i],
        maxlon = box$max_lon[i]) %>%
        mutate(index = i) %>% mutate(year = box$year[i])
      if(nrow(individuals) == 0)
      {
        non_existent <- tibble(site_number = NA, index = i)
        monitors <- bind_rows(monitors, non_existent)
        next
      }
      monitors <- bind_rows(monitors, individuals)
    }
    monitors_cleaned <- monitors %>% filter(!is.na(latitude) | !is.na(longitude))
    monitor_coords <- monitors_cleaned %>% select(index, site_number, lat = latitude, lon = longitude, state_code, county_code, address, year)
    monitors_sf <- monitor_coords %>%
      st_as_sf(coords = c("lon", "lat"), crs = 4326) %>% distinct()
    address_sf <- coords %>%
      st_as_sf(coords = c("lon", "lat"), crs = 4326) %>% mutate(index = row_number()) %>% mutate(year = year)

    to_delete_monitors <- monitors %>%
      filter(!is.na(index) & is.na(site_number)) %>% select(index)
    invalids_removed <- address_sf %>%
      filter(!(index %in% to_delete_monitors$index))

    list <- tibble()
    data <- tibble()

    for(i in 1:nrow(invalids_removed))
    {
      monitors_sf_distance <- monitors_sf %>%
        mutate(distance_m = st_distance(geometry, invalids_removed$geometry[i]))
      closest_monitor <- monitors_sf_distance %>%
        arrange(distance_m) %>%
        filter(index == invalids_removed$index[i]) %>%
        slice(1) %>%
        st_drop_geometry() %>%
        select(index, site_number, state_code, county_code, year)
      list <- list %>% bind_rows(closest_monitor)
      site_data <- aqs_annualsummary_by_site(
        parameter = c(pm = "88101", co = "42101", no2 = "42602", o3 = "44201", so2 = "42401"),
        bdate = as.Date(paste0(closest_monitor$year, "0101"),format = "%Y%m%d"),
        edate = as.Date(paste0(closest_monitor$year, "1231"),format = "%Y%m%d"),
        stateFIPS = as.character(closest_monitor$state_code),
        countycode = as.character(closest_monitor$county_code),
        sitenum = as.character(closest_monitor$site_number)
      ) %>%
        mutate(index = invalids_removed$index[i]) %>%
        mutate(aqi = case_when(

          # PM2.5 (µg/m³)
          parameter_code == "88101" & arithmetic_mean >= 0.0   & arithmetic_mean <= 12.0   ~ ((50 - 0)    / (12.0 - 0.0))   * (arithmetic_mean - 0.0)    + 0,
          parameter_code == "88101" & arithmetic_mean > 12.0   & arithmetic_mean <= 35.4   ~ ((100 - 51)  / (35.4 - 12.1))  * (arithmetic_mean - 12.1)   + 51,
          parameter_code == "88101" & arithmetic_mean > 35.4   & arithmetic_mean <= 55.4   ~ ((150 - 101) / (55.4 - 35.5))  * (arithmetic_mean - 35.5)   + 101,
          parameter_code == "88101" & arithmetic_mean > 55.4   & arithmetic_mean <= 150.4  ~ ((200 - 151) / (150.4 - 55.5)) * (arithmetic_mean - 55.5)   + 151,
          parameter_code == "88101" & arithmetic_mean > 150.4  & arithmetic_mean <= 250.4  ~ ((300 - 201) / (250.4 - 150.5))* (arithmetic_mean - 150.5)  + 201,
          parameter_code == "88101" & arithmetic_mean > 250.4  & arithmetic_mean <= 350.4  ~ ((400 - 301) / (350.4 - 250.5))* (arithmetic_mean - 250.5)  + 301,
          parameter_code == "88101" & arithmetic_mean > 350.4  & arithmetic_mean <= 500.4  ~ ((500 - 401) / (500.4 - 350.5))* (arithmetic_mean - 350.5)  + 401,
          # Ozone (ppm, 8-hour)
          parameter_code == "44201" & arithmetic_mean >= 0.000 & arithmetic_mean <= 0.054 ~ ((50 - 0)   / (0.054 - 0.000)) * (arithmetic_mean - 0.000) + 0,
          parameter_code == "44201" & arithmetic_mean > 0.054  & arithmetic_mean <= 0.070 ~ ((100 - 51) / (0.070 - 0.055)) * (arithmetic_mean - 0.055) + 51,
          parameter_code == "44201" & arithmetic_mean > 0.070  & arithmetic_mean <= 0.085 ~ ((150 - 101)/ (0.085 - 0.071)) * (arithmetic_mean - 0.071) + 101,
          parameter_code == "44201" & arithmetic_mean > 0.085  & arithmetic_mean <= 0.105 ~ ((200 - 151)/ (0.105 - 0.086)) * (arithmetic_mean - 0.086) + 151,
          parameter_code == "44201" & arithmetic_mean > 0.105  & arithmetic_mean <= 0.200 ~ ((300 - 201)/ (0.200 - 0.106)) * (arithmetic_mean - 0.106) + 201,
          # Carbon Monoxide (ppm)
          parameter_code == "42101" & arithmetic_mean >= 0.0   & arithmetic_mean <= 4.4   ~ ((50 - 0)   / (4.4 - 0.0))   * (arithmetic_mean - 0.0)   + 0,
          parameter_code == "42101" & arithmetic_mean > 4.4    & arithmetic_mean <= 9.4   ~ ((100 - 51)/ (9.4 - 4.5))   * (arithmetic_mean - 4.5)   + 51,
          parameter_code == "42101" & arithmetic_mean > 9.4    & arithmetic_mean <= 12.4  ~ ((150 - 101)/ (12.4 - 9.5)) * (arithmetic_mean - 9.5)   + 101,
          parameter_code == "42101" & arithmetic_mean > 12.4   & arithmetic_mean <= 15.4  ~ ((200 - 151)/ (15.4 - 12.5))* (arithmetic_mean - 12.5)  + 151,
          parameter_code == "42101" & arithmetic_mean > 15.4   & arithmetic_mean <= 30.4  ~ ((300 - 201)/ (30.4 - 15.5))* (arithmetic_mean - 15.5)  + 201,
          parameter_code == "42101" & arithmetic_mean > 30.4   & arithmetic_mean <= 40.4  ~ ((400 - 301)/ (40.4 - 30.5))* (arithmetic_mean - 30.5)  + 301,
          parameter_code == "42101" & arithmetic_mean > 40.4   & arithmetic_mean <= 50.4  ~ ((500 - 401)/ (50.4 - 40.5))* (arithmetic_mean - 40.5)  + 401,
          # Nitrogen Dioxide (NO₂, ppb)
          parameter_code == "42602" & arithmetic_mean >= 0     & arithmetic_mean <= 53    ~ ((50 - 0)   / (53 - 0))    * (arithmetic_mean - 0)    + 0,
          parameter_code == "42602" & arithmetic_mean > 53     & arithmetic_mean <= 100   ~ ((100 - 51)/ (100 - 54))   * (arithmetic_mean - 54)   + 51,
          parameter_code == "42602" & arithmetic_mean > 100    & arithmetic_mean <= 360   ~ ((150 - 101)/ (360 - 101)) * (arithmetic_mean - 101)  + 101,
          parameter_code == "42602" & arithmetic_mean > 360    & arithmetic_mean <= 649   ~ ((200 - 151)/ (649 - 361)) * (arithmetic_mean - 361)  + 151,
          parameter_code == "42602" & arithmetic_mean > 649    & arithmetic_mean <= 1249  ~ ((300 - 201)/ (1249 - 650))* (arithmetic_mean - 650)  + 201,
          parameter_code == "42602" & arithmetic_mean > 1249   & arithmetic_mean <= 1649  ~ ((400 - 301)/ (1649 - 1250))* (arithmetic_mean - 1250)+ 301,
          parameter_code == "42602" & arithmetic_mean > 1649   & arithmetic_mean <= 2049  ~ ((500 - 401)/ (2049 - 1650))* (arithmetic_mean - 1650)+ 401,
          # Sulfur Dioxide (SO₂, ppb)
          parameter_code == "42401" & arithmetic_mean >= 0     & arithmetic_mean <= 35    ~ ((50 - 0)   / (35 - 0))    * (arithmetic_mean - 0)    + 0,
          parameter_code == "42401" & arithmetic_mean > 35     & arithmetic_mean <= 75    ~ ((100 - 51)/ (75 - 36))    * (arithmetic_mean - 36)   + 51,
          parameter_code == "42401" & arithmetic_mean > 75     & arithmetic_mean <= 185   ~ ((150 - 101)/ (185 - 76))  * (arithmetic_mean - 76)   + 101,
          parameter_code == "42401" & arithmetic_mean > 185    & arithmetic_mean <= 304   ~ ((200 - 151)/ (304 - 186)) * (arithmetic_mean - 186)  + 151,
          parameter_code == "42401" & arithmetic_mean > 304    & arithmetic_mean <= 604   ~ ((300 - 201)/ (604 - 305)) * (arithmetic_mean - 305)  + 201,
          parameter_code == "42401" & arithmetic_mean > 604    & arithmetic_mean <= 804   ~ ((400 - 301)/ (804 - 605)) * (arithmetic_mean - 605)  + 301,
          parameter_code == "42401" & arithmetic_mean > 804    & arithmetic_mean <= 1004  ~ ((500 - 401)/ (1004 - 805))* (arithmetic_mean - 805)  + 401,
          TRUE ~ NA_real_)) %>%
        select(state, state_code, county, county_code, year, site_number, site_address, aqi, index, parameter_code) %>%
                  group_by(state, state_code, county, county_code, year, site_number, site_address) %>%
         slice_max(order_by = aqi, n = 1) %>% ungroup() %>% distinct()
      data <- bind_rows(data, site_data)
    }
    address_sf <- address_sf %>%
      left_join(data %>% select(year, index, aqi), by = c("year", "index")) %>% st_drop_geometry() %>%
      mutate(category_AQI = case_when(
        aqi >= 0 & aqi <= 50 ~ "Good",
    ...
    tib <- tib %>%
      left_join(address_sf %>% select(address, year, AQI = aqi, category_AQI), by = c("address", "year"))
    return(tib)
}
...
{
  atlas_stores <- read_excel("FoodEnvironmentAtlas.xls", sheet = "STORES")
  atlas_restaurants <- read_excel("FoodEnvironmentAtlas.xls", sheet = "RESTAURANTS")
...
{
  clean_twenty_four <- read_csv("EJ_index_data/EJSCREEN_2024.csv", show_col_types = FALSE) %>%
    mutate(across(everything(), as.character)) %>%
    mutate(NATA_cancer = NA) %>%
    mutate(NATA_resp_haz = NA) %>%
    select(GEOID = ID, particulate_matter = D5_PM25, ozone = D5_OZONE, diesal_pm = D5_DSLPM, traffic_prox_vol = D5_PTRAF, lead_paint = D5_LDPNT, prox_to_NPLS = D5_PNPL, prox_to_RMPS = D5_PRMP, prox_to_TSDF = D5_PTSDF, prox_to_MDWD = D5_PWDIS)
  clean_twenty_three <- read_csv("EJ_index_data/EJSCREEN_2023.csv", show_col_types = FALSE) %>%
    mutate(across(everything(), as.character)) %>%
    select(GEOID = ID, particulate_matter = D5_PM25, ozone = D5_OZONE, diesal_pm = D5_DSLPM, traffic_prox_vol = D5_PTRAF, lead_paint = D5_LDPNT, prox_to_NPLS = D5_PNPL, prox_to_RMPS = D5_PRMP, prox_to_TSDF = D5_PTSDF, prox_to_MDWD = D5_PWDIS, NATA_cancer = D5_CANCER, NATA_resp_haz = D5_RESP)
  clean_twenty_two <- read_csv("EJ_index_data/EJSCREEN_2022.csv", show_col_types = FALSE) %>%
    mutate(across(everything(), as.character)) %>%
    select(GEOID = ID, particulate_matter = D_PM25_2, ozone = D_OZONE_2, diesal_pm = D_DSLPM_2, traffic_prox_vol = D_PTRAF_2, lead_paint = D_LDPNT_2, prox_to_NPLS = D_PNPL_2, prox_to_RMPS = D_PRMP_2, prox_to_TSDF = D_PTSDF_2, prox_to_MDWD = D_PWDIS_2, NATA_cancer = D_CANCR_2, NATA_resp_haz = D_RESP_2)
  clean_twenty_one <- read_csv("EJ_index_data/EJSCREEN_2021.csv", show_col_types = FALSE) %>%
    mutate(across(everything(), as.character)) %>%
    select(GEOID = ID, particulate_matter = D_PM25_2, ozone = D_OZONE_2, diesal_pm = D_DSLPM_2, traffic_prox_vol = D_PTRAF_2, lead_paint = D_LDPNT_2, prox_to_NPLS = D_PNPL_2, prox_to_RMPS = D_PRMP_2, prox_to_TSDF = D_PTSDF_2, prox_to_MDWD = D_PWDIS_2, NATA_cancer = D_CANCR_2, NATA_resp_haz = D_RESP_2)
  clean_twenty <- read_csv("EJ_index_data/EJSCREEN_2020.csv", show_col_types = FALSE) %>%
    mutate(across(everything(), as.character)) %>%
    select(GEOID = ID, particulate_matter = D_PM25_2, ozone = D_OZONE_2, diesal_pm = D_DSLPM_2, traffic_prox_vol = D_PTRAF_2, lead_paint = D_LDPNT_2, prox_to_NPLS = D_PNPL_2, prox_to_RMPS = D_PRMP_2, prox_to_TSDF = D_PTSDF_2, prox_to_MDWD = D_PWDIS_2, NATA_cancer = D_CANCR_2, NATA_resp_haz = D_RESP_2)
  clean_nineteen <- read_csv("EJ_index_data/EJSCREEN_2019.csv", show_col_types = FALSE) %>%
    mutate(across(everything(), as.character)) %>%
    select(GEOID = ID, particulate_matter = D_PM25_2, ozone = D_OZONE_2, diesal_pm = D_DSLPM_2, traffic_prox_vol = D_PTRAF_2, lead_paint = D_LDPNT_2, prox_to_NPLS = D_PNPL_2, prox_to_RMPS = D_PRMP_2, prox_to_TSDF = D_PTSDF_2, prox_to_MDWD = D_PWDIS_2, NATA_cancer = D_CANCR_2, NATA_resp_haz = D_RESP_2)
  clean_eighteen <- read_csv("EJ_index_data/EJSCREEN_2018.csv", show_col_types = FALSE) %>%
    mutate(across(everything(), as.character)) %>%
    select(GEOID = ID, particulate_matter = D_PM25_2, ozone = D_OZONE_2, diesal_pm = D_DSLPM_2, traffic_prox_vol = D_PTRAF_2, lead_paint = D_LDPNT_2, prox_to_NPLS = D_PNPL_2, prox_to_RMPS = D_PRMP_2, prox_to_TSDF = D_PTSDF_2, prox_to_MDWD = D_PWDIS_2, NATA_cancer = D_CANCR_2, NATA_resp_haz = D_RESP_2)
  clean_seventeen <- read_csv("EJ_index_data/EJSCREEN_2017.csv", show_col_types = FALSE) %>%
    mutate(across(everything(), as.character)) %>%
    select(GEOID = ID, particulate_matter = D_PM25_2, ozone = D_OZONE_2, diesal_pm = D_DSLPM_2, traffic_prox_vol = D_PTRAF_2, lead_paint = D_LDPNT_2, prox_to_NPLS = D_PNPL_2, prox_to_RMPS = D_PRMP_2, prox_to_TSDF = D_PTSDF_2, prox_to_MDWD = D_PWDIS_2, NATA_cancer = D_CANCR_2, NATA_resp_haz = D_RESP_2)
  clean_sixteen <- read_csv("EJ_index_data/EJSCREEN_2016.csv", show_col_types = FALSE) %>%
    mutate(across(everything(), as.character)) %>%
    select(GEOID = ID, particulate_matter = D_PM25_2, ozone = D_OZONE_2, diesal_pm = D_DSLPM_2, traffic_prox_vol = D_PTRAF_2, lead_paint = D_LDPNT_2, prox_to_NPLS = D_PNPL_2, prox_to_RMPS = D_PRMP_2, prox_to_TSDF = D_PTSDF_2, prox_to_MDWD = D_PWDIS_2, NATA_cancer = D_CANCR_2, NATA_resp_haz = D_RESP_2)
...
{
  consolidated <- read.csv("min_wage/consol_mw.csv", check.names = FALSE)
  change <- read.csv("min_wage/chg_in_mw.csv", check.names = FALSE)
...
{
  union_data <- read.csv("union/union_data.csv")
  tib <- tib %>%
...
#' **tidyverse**, **tidygeocoder**, **tibble**, **tidycensus**(API KEY), **tigris**, **sf**,
#' **RAQSAPI**(API KEY), **readxl**
#'
...
get_geodeterminants <- function(gd_tib = NULL, gd_addresses = NULL, gd_current_year = 2025, gd_minority_group_code = NULL, gd_comparison_group_code = NULL, gd_minority_group_code_dhc = NULL, gd_minority_group_code_sf1 = NULL, gd_current_fed_min_wage = 7.25)
{
  ...
  else if(is.null(gd_addresses))
  {
    ...
    gd_tib <- get_GEOID(tib = gd_tib)
    gd_tib <- add_info_cols(tib = gd_tib)
    gd_tib <- get_county_geo(tib = gd_tib)
  }
  else if(is.null(gd_tib))
  {
    geocoded <- vector_geocode_to_tib(addresses = gd_addresses)
    gd_tib <- SDOH_reverse(geo_tib = geocoded)
    gd_tib <- gd_tib %>%
      mutate(actual_year = NA) %>%
      mutate(year = gd_current_year - 2)
    gd_tib <- get_GEOID(tib = gd_tib)
    gd_tib <- get_county_geo(tib = gd_tib)
  }
  else
