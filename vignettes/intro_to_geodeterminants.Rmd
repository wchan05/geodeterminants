...
test_tib <- tibble(
  ...
  state = c("NJ", "MN", "MO", "TX", "CA", "MA", "PA", "NJ", "MI"),
  year = c(2022, 2023, NA, 2023, NA, 2023, 2022, 2022, NA))
...
tib_result <- get_geodeterminants(test_tib)
...
vec_result <- get_geodeterminants(test_vec)
...
```{r, message=FALSE, warning=FALSE}
gd_minwag <- add_min_wage(tib = gd_tib, current_fed_min_wage = gd_current_fed_min_wage)
head(gd_minwag)
```  
...
```{r, message=FALSE, warning=FALSE}
gd_pu <- add_pct_unionized(tib = gd_tib, current_year = gd_current_year)
head(pu)
``` 
